...
#             return spacecraft
# NOTE TO SELF, OLDER PYTHON VERSION DONT SUPPORT THIS OR TYPE ANNOTATION
@router.get("/api/spacecrafts/{spacecraft_id}", tags=["Spacecrafts"], response_model=Spacecraft)
def get_spacecraft(spacecraft_id: int, show_id: str = Query("Default", description="Show spacecraft id")):
    for spacecraft in spacecrafts:
        if spacecraft["id"] == spacecraft_id:
            if show_id != "Default":
                return {"name": spacecraft["name"]}
